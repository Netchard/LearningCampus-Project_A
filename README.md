# LearningCampus-Project_A
Learning Campus - Formation Vide Coding - Projet A - Atelier Nova

# Projet A
- Sequence 3 - Structure de la landing page  [Voir la page](./atelier-nova-landing-page-premium-minimaliste(3).html)
- Sequence 4 - Construire la landing page V1 [Voir la page](./atelier-nova-landing-page-premium-minimaliste(4).html)
- Sequence 5 - Construire la landing page V2 [Voir la page](./atelier-nova-landing-page-premium-minimaliste(8).html)
- Sequence 6 - Construire la landing page V3 [Voir la page](./atelier-nova-landing-page-premium-minimaliste(11).html)
  
# Site généré avec Lovable 
- [Voir le site](https://urban-capsule-vip.lovable.app/)


# Performance PageSpeed
- Page Html simple [voit le résultat](https://pagespeed.web.dev/analysis/https-netchard-github-io-LearningCampus-Project_A-atelier-nova-landing-page-premium-minimaliste(11)-html/qimyn8e1j9?form_factor=desktop&category=performance&category=accessibility&category=best-practices&category=seo&hl=fr&utm_source=lh-chrome-ext)
- Page Lovable  [voit le résultat](https://pagespeed.web.dev/analysis/https-urban-capsule-vip-lovable-app/7akyvtpln4?form_factor=desktop&category=performance&category=accessibility&category=best-practices&category=seo&hl=fr&utm_source=lh-chrome-ext)
